...
User.schema.virtual('avatarUrl').get(() => {
  ...
  if (this.gravatar) {
    return `https://www.gravatar.com/avatar/${this.gravatar}?d=https%3A%2F%2Fsydjs.com%2Fimages%2Favatar.png&r=pg`;
  }
});

// Usernames
User.schema.virtual('twitterUsername').get(() => ((this.services.twitter && this.services.twitter.isConfigured) ? this.services.twitter.username : ''));
User.schema.virtual('githubUsername').get(() => ((this.services.github && this.services.github.isConfigured) ? this.services.github.username : ''));
...
User.schema.methods.resetPassword = (callback) => {
  const user = this;
